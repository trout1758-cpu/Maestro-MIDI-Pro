    ...
    exportXML() {
        ...
        State.parts.forEach((part, index) => {
            ...
            sortedItems.sort((a, b) => {
                ...
                if (Math.abs(a.x - b.x) < 5.0) {
                    const getPriority = (type) => {
                        ...
                        if (type === 'tempo') return 1.4; // Tempo usually at start
            ...
            for (let i = 0; i < sortedItems.length; i++) {
                ...
                // --- NEW: EXPORT TEMPO ---
                if (note.type === 'tempo') {
                    // Map internal ID to XML types
                    let unit = 'quarter';
                    let dotted = false;
                    
                    if (note.duration == 2) unit = 'half';
                    if (note.duration == 4) unit = 'quarter';
                    if (note.duration == 8) unit = 'eighth';
                    if (note.duration == 4.5) { unit = 'quarter'; dotted = true; }
                    if (note.duration == 2.5) { unit = 'half'; dotted = true; }

                    xml += `      <direction placement="above">
        <direction-type>
          <metronome>
            <beat-unit>${unit}</beat-unit>\n`;
                    if (dotted) xml += `            <beat-unit-dot/>\n`;
                    xml += `            <per-minute>${note.bpm}</per-minute>
          </metronome>
        </direction-type>
        <sound tempo="${note.bpm}"/>
      </direction>\n`;
                    continue;
                }
